...
typedef struct RelOptInfo
{
	...
	/* 本RelOptInfo包含的所有关系 all relations included in this RelOptInfo */
	Relids		relids;			/* 基本关系的ID集合 (范围表索引) set of base relids (rangetable indexes) */

	/* 由计划器生成的预估尺寸 size estimates generated by planner */
	double		rows;			/* 预估结果元组数目 estimated number of result tuples */

	/* 计划器标记位，每个关系一份 per-relation planner control flags */
	bool		consider_startup;		/* 保留启动代价最低的路径 keep cheap-startup-cost paths? */
	bool		consider_param_startup; /* 同上, 针对参数化路径 ditto, for parameterized paths? */
	bool		consider_parallel;		/* 考虑并行路径 consider parallel paths? */

	/* 扫描当前关系的默认结果目标列表 default result targetlist for Paths scanning this relation */
	struct PathTarget *reltarget;		/* Vars/Exprs, 代价, 宽度的列表 list of Vars/Exprs, cost, width */

	/* 物化相关信息 materialization information */
	List	   *pathlist;		/* Path 结构体列表 Path structures */
	List	   *ppilist;		/* pathlist中使用的ParamPathInfos ParamPathInfos used in pathlist */
	List	   *partial_pathlist;		/* 部分路径 partial Paths */
	struct Path *cheapest_startup_path;
	struct Path *cheapest_total_path;
	struct Path *cheapest_unique_path;
	List	   *cheapest_parameterized_paths;

	/* 基础关系与连接关系都需要的 参数化信息 parameterization information needed for both base rels and join rels */
	/* (参见 lateral_vars 与 lateral_referencers) (see also lateral_vars and lateral_referencers) */
	Relids		direct_lateral_relids;	/* 直接以LATERAL方式引用的关系 rels directly laterally referenced */
	Relids		lateral_relids; /* minimum parameterization of rel */

	/* 关于基础关系的信息 (连接关系不会设置这些字段！) information about a base rel (not set for join rels!) */
	Index		relid;
	Oid			reltablespace;	/* 表空间 containing tablespace */
	RTEKind		rtekind;		/* RELATION, SUBQUERY, 或 FUNCTION RELATION, SUBQUERY, or FUNCTION */
	AttrNumber	min_attr;		/* 关系属性的最小值 (通常<0) smallest attrno of rel (often <0) */
	AttrNumber	max_attr;		/* 关系属性的最大值 largest attrno of rel */
	Relids	   *attr_needed;	/* 被索引的数组 [min_attr .. max_attr] array indexed [min_attr .. max_attr] */
	int32	   *attr_widths;	/* 被索引的数组 [min_attr .. max_attr] array indexed [min_attr .. max_attr] */
	List	   *lateral_vars;	/* 关系所引用的LATERAL Vars 与 PHVs LATERAL Vars and PHVs referenced by rel */
	Relids		lateral_referencers;	/* 侧面引用本表的关系 rels that reference me laterally */
	List	   *indexlist;		/* IndexOptInfo列表 list of IndexOptInfo */
	BlockNumber pages;			/* 来自pg_class的页面数估计值 size estimates derived from pg_class */
	double		tuples;
	double		allvisfrac;
	PlannerInfo *subroot;		/* 如有子查询 if subquery */
	List	   *subplan_params; /* 如有子查询 if subquery */
	int			rel_parallel_workers;	/* 期望的并行工作进程数量 wanted number of parallel workers */

	/* 有关外部表与外部表连接的相关信息 Information about foreign tables and foreign joins */
	Oid			serverid;		/* 外部表与外部表连接相应的服务器ID identifies server for the table or join */
	Oid			userid;			/* 用于检查访问权限的用户标识 identifies user to check access as */
	bool		useridiscurrent;	/* 当前用户是否能合法进行JOIN join is only valid for current user */
	/* use "struct FdwRoutine" to avoid including fdwapi.h here */
	struct FdwRoutine *fdwroutine;
	void	   *fdw_private;

	/* 被各种扫描与连接所使用 used by various scans and joins: */
	List	   *baserestrictinfo;		/* RestrictInfo结构体列表 (如果存在基础关系)  RestrictInfo structures (if base
										 * rel) */
	QualCost	baserestrictcost;		/* 求值上述限制条件的代价 cost of evaluating the above */
	List	   *joininfo;		/* RestrictInfo 结构体列表，涉及到本表的连接会用到 RestrictInfo structures for join clauses
								 * involving this rel */
	bool		has_eclass_joins;		/* T 意味着joininfo不完整 T means joininfo is incomplete */
} RelOptInfo;
